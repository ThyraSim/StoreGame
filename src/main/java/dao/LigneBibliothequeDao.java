...
import entities.LigneBibliotheque;
...
public class LigneBibliothequeDao {

    public static void insert(LigneBibliotheque ligneBibliotheque){
        ...
        try{
            ...
            entityManager.persist(ligneBibliotheque);
    ...
    public static boolean delete(Integer id) {
        ...
        try {
            ...
            entityManager.remove(entityManager.find(LigneBibliotheque.class, id));
            entityManager.getTransaction().commit();
            return true;
        } catch (Exception e) {
            entityManager.getTransaction().rollback();
    ...
    public static List<LigneBibliotheque> findAll() {
        EntityManagerFactory entityManagerFactory =
                Persistence.createEntityManagerFactory("connection");
        EntityManager entityManager = entityManagerFactory.createEntityManager();

        Query query = entityManager.createQuery("select i from LigneBibliotheque i");
        return query.getResultList();
    }

    public static List<LigneBibliotheque> findLignesBibliothequeByBiblioId(int idBiblio) {
        EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("connection");
        EntityManager entityManager = entityManagerFactory.createEntityManager();

        String jpql = "SELECT b FROM LigneBibliotheque b " +
                "JOIN l.bibliotheque c " +
                "WHERE b.id = :idBiblio AND l.panier = false";

        TypedQuery<Bibliotheque> query = entityManager.createQuery(jpql, Bibliotheque.class);
        query.setParameter("idBiblio", idBiblio);
    ...
    public static void afficherListeLignesBibliotheque() {
        List<LigneBibliotheque> ligneBibliothequeList = LigneBibliothequeDao.findAll();
        for(LigneBibliotheque ligneBibliotheque: ligneBibliothequeList) {
            System.out.print(ligneBibliotheque + " ");
        }
    }
